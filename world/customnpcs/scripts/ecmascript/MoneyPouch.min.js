...
var opening = false;
var ITEM_TEXTURE_ID = 411;
var afterInit = false;

function init(e) {
    var itemNbt = e.item.getNbt();

    if (!itemNbt.has('min')) {
        storeddata.setFloat('min', 10);
    }
    if (!itemNbt.has('max')) {
        storeddata.setFloat('max', 20000);
    }
    e.item.setDurabilityShow(false);
    e.item.setTexture(ITEM_TEXTURE_ID, 'variedcommodities:blueprint');
    e.item.setItemDamage(ITEM_TEXTURE_ID);
    e.item.setColor(parseInt('d7b8ff', 16));

    e.item.setCustomName(ccs('&3&lLottery Print'));
}
...
function interact(e) {
    if (!opening) {
        ...
        var _min = itemNbt.getFloat('min') || 10;
        var _max = itemNbt.getFloat('max') || 20000;

        var min = Math.min(_min, _max);
        var max = Math.max(_min, _max);

        var amount = rrandom_range(min, max);
        showPlayerMoneyEvent(e, amount, 0, max.toString().length);
    }
}

function showPlayerMoneyEvent(e, amount, current, maxLen) {
	if(typeof(current) == typeof(undefined) || current === null) { current = 0; }
	if(typeof(maxLen) == typeof(undefined) || maxLen === null) { maxLen = null; }
    var output = ':money:&e';
    var amountLength = amount.toString().length;
    var maxLen = maxLen == null ? amountLength : maxLen;
    var amountIndex = 0;
    for (var i = maxLen - 1; i >= 0; i--) {
        if (i < current) {
            output += '&e';
            if (i <= amountLength - 1) {
                output += amount.toString()[amountIndex];
            } else {
                output += '0';
            }
        } else {
    ...
    if (current < maxLen) {
        runDelay(0.7, function() {
            current++;
            executeCommand(e.player, '/playsound minecraft:ui.button.click ambient ' + e.player.getName() + ' ~ ~ ~ 1 1.5');
            showPlayerMoneyEvent(e, amount, current, maxLen);
        });
    } else {
        runDelay(0.7, function() {
            executeCommand(e.player, '/playsound minecraft:ui.toast.challenge_complete ambient ' + e.player.getName() + ' ~ ~ ~ 1 2');
            var data = e.player.world.storeddata;
            new Player(e.player.getName()).init(data).add('money', amount * 100).save(data);
            var showAmount = getAmountCoin(getCoinAmount((amount * 100).toString() + 'C'));
            tellPlayer(e.player, '&aAdded &r:money:&e' + showAmount + ' &ato your money pouch.');

            e.item.setStackSize(e.item.getStackSize() - 1);
            opening = false;
        });
    }
}
...
function tick(e) {
    ...
    if (!afterInit) {
        var itemNbt = e.item.getNbt();
        var min = getAmountCoin(itemNbt.getFloat('min') * 100);
        var max = getAmountCoin(itemNbt.getFloat('max') * 100);
        e.item.setLore([
            ccs(parseEmotes('&bContains &r:money:&e' + min + '&b - &r:money:&e' + max)),
        ]);
        afterInit = true;
    }
}