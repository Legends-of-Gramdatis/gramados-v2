...
registerXCommands([
    ...
    ['!loottableVLP forceadd <alias> <weight>', function (pl, args, data) {
        var mainhand = pl.getMainhandItem();
        ...
        if (!result.Success) {
            switch (result.Reason) {
                ...
                case 'invalid_whitelist':
                    ...
                    break;
                case 'write_failed':
                    tellPlayer(pl, '&c:cross: Failed to force-add the item to volatile loot pool &e' + result.Alias + '&c. The item was not consumed.');
                    break;
                default:
                    tellPlayer(pl, '&c:cross: Failed to force-add the item to the volatile loot pool. The item was not consumed.');
                    break;
            }
