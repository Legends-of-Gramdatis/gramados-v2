...
var TICKET_DATA_PATH = "/home/mouette/gramados-v2/world/customnpcs/scripts/data_auto/mafia_tickets.json";

function loadMafiaData() {
    return checkFileExists(MAFIA_DATA_PATH) ? loadJson(MAFIA_DATA_PATH) : {};
}
...
function loadTicketData() {
    return checkFileExists(TICKET_DATA_PATH) ? loadJson(TICKET_DATA_PATH) : { tickets: [] };
}

function saveTicketData(data) {
    saveJson(data, TICKET_DATA_PATH);
}
...
function interact(event) {
    ...
    var ticketData = loadTicketData();
    var mafiaData = loadJson(MAFIA_DATA_PATH);
    var playerName = player.getName();
    var currentTime = getAgeTick(world);

    if (!mafiaData[playerName]) {
        mafiaData[playerName] = { lastTicketTime: 0 };
    }

    var lastTicketTime = mafiaData[playerName].lastTicketTime;
    ...
    if (!item.isEmpty() && item.getName() === "minecraft:paper") {
        ...
        var ticket = null;
        for (var j = 0; j < ticketData.tickets.length; j++) {
            if (ticketData.tickets[j].id === ticketId) {
                ticket = ticketData.tickets[j];
                break;
            }
        }
        ...
        logToFile("mafia", player.getName() + " gained " + repGain + " reputation for " + (costInCents / 100) + "g.");
        return;
    }
    ...
    ticketItem.setLore([
        ...
        parseEmotes("§7Cost: §r:money:§e" + getAmountCoin(costInG)),
    ...
    mafiaData[playerName].lastTicketTime = currentTime;
    saveMafiaData(mafiaData);

    logToFile("mafia", player.getName() + " generated a ticket worth " + repToOffer + " reputation for " + getAmountCoin(costInG) + ".");
}
