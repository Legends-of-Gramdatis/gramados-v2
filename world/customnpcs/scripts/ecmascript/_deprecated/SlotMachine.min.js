...
function genMoneyItem(w, amount, currencyType) {
	if(typeof(currencyType) == typeof(undefined) || currencyType === null) { currencyType = 'money'; }
    var currency = getCurrency(currencyType);
    var itemTexture = null;
    for(var itemAmount in _COINITEMS) {
var coinItem = _COINITEMS[itemAmount];
        if(amount >= getCoinAmount(itemAmount)) {
            itemTexture = coinItem;
        }
    }

    var item = w.createItem(itemTexture || 'minecraft:gold_nugget', 0, 1);
    item.setCustomName(ccs(currency.displayPrefix + currency.displayName + '&r'));
    item.setLore([
        ccs('&e' + getAmountCoin(amount))
    ]);
...
var opening = false;
var ITEM_TEXTURE_ID = 412;
var rolled = [];
var rollAmount = 3; //Amount of emotes to roll
var rollStartIndex = 0; //Randomizes per roll, on what emote it starts on, before rolling

var rollTicks = 10; //Randomizes per roll, amount of steps that it should roll
var rollTicksMin = 6;
var rollTicksMax = 12;

var rollTickSpeed = 0.1; //Increases per tick
var rollTickSpeedIncrease = .05;
var rollLength = 2; //Emote rollback display length left and right each
var rollCost = '10G';

function rollInit(e) {
    rollStartIndex = Math.round(Math.random() * (emotes.length - 1));
    rollTicks = rrandom_range(rollTicksMin, rollTicksMax);
}
...
var showRewards = rewards.concat([]);

rewards.sort(function(a, b) {
    if (a.pattern.length < b.pattern.length) { return -1; }
    if (a.pattern.length > b.pattern.length) { return 1; }

    var anull = Array.count(a.pattern, null);
    var bnull = Array.count(b.pattern, null);
...
function interact(e) {
    ...
    if (!e.player.isSneaking()) {
        if (!opening) {
            var p = new Player(e.player.getName()).init(data);
            var rollCostAmount = getCoinAmount(rollCost);
            if (p.data.amoney < rollCostAmount) {
                tellPlayer(e.player, '&cYou need ' + formatCurrency(rollCostAmount, 'amoney') + ' &cto roll.');
                return false;
            }
            rolled = [];
            rollInit(e);
            opening = e.player.getName();
            p.data.amoney -= rollCostAmount;
            p.save(data);
            rollEvent(e, rollStartIndex, rollTicks, rollTickSpeed);
        } else {
            tellPlayer(e.player, '&cI am already opening for ' + opening + ', please wait.');
        }
    } else {
        var output = getTitleBar('SlotMachine Rewards', false) + '\n';
        for(var i in showRewards) {
var rewardData = showRewards[i];
            var rewardStr = '&r[';
            for(var j in rewardData.pattern) {
var pattern = rewardData.pattern[j];
                rewardStr += pattern ? ':' + pattern + ':' : '&8:sun:{*|show_text:$aThis can be anything else}&r';
            }
            rewardStr += '&r]';
            var rewardText = (rewardData.shapeless ? '&cShapeless pattern (= it only has to contain it)\n' : '&cShaped pattern (= result must be this pattern)\n') +
                (rewardData.mirrored ? '&cMirrored pattern (this pattern works also mirrored)\n' : 'Non-Mirrored pattern (this pattern doesn\'t work mirrored)\n');
            for(var type in rewardData.rewards) {
var reward = rewardData.rewards[type];
                switch (type) {
                    case 'credit':
                    case 'amoney':
                    case 'vmoney':
                    case 'money':
                        rewardText += '&aGives &r' + formatCurrency(reward, type) + '\n';
                        break;
                }
            }

            output += '&r' + rewardStr + '&5[?]{*|show_text:' + ccs(rewardText || '.') + '}&r ' + (parseInt(i) != 0 && (i + 1) % 3 == 0 ? '\n' : '');
        }


        tellPlayer(e.player, output);
        // tellPlayer(e.player, formatObj(rewards));
    }
}
...
function rollEvent(e, currentIndex, ticksLeft, tickTime) {
	if(typeof(currentIndex) == typeof(undefined) || currentIndex === null) { currentIndex = null; }
	if(typeof(ticksLeft) == typeof(undefined) || ticksLeft === null) { ticksLeft = null; }
	if(typeof(tickTime) == typeof(undefined) || tickTime === null) { tickTime = null; }
    currentIndex = (currentIndex == null ? rollStartIndex : currentIndex);
    tickTime = tickTime == null ? rollTickSpeed : tickTime;
    ticksLeft = ticksLeft == null ? rollTicks : ticksLeft;
    ...
    if (rolled.length < rollAmount) {
        var showEmotes = getArrayRange(emotes, currentIndex, rollLength);

        var output = '';
        for (var i in showEmotes) {
            var isMiddle = parseInt(i) == rollLength;
            if (isMiddle) {
                output += (ticksLeft <= 0 ? '&b' : '') + '[&r';
            }
            output += ':' + showEmotes[i] + ':';
            if (isMiddle) {
                output += (ticksLeft <= 0 ? '&b' : '') + ']&r';
        ...
        tellPlayerTitle(e.player, subtitle, 'subtitle');
        tellPlayerTitle(e.player, output, 'title');
        if (ticksLeft > 0) {
            currentIndex = (currentIndex + 1) % emotes.length;
            runDelay(tickTime, function() {
                rollEvent(e, currentIndex, ticksLeft - 1, Math.min(2, tickTime + rollTickSpeedIncrease));
            });
        } else {
            // DEBUG RESULT
            // rolled.push(['brick', 'emerald', 'emerald'][rolled.length]);
            rolled.push(emotes[currentIndex]);
        ...
        var giveRewards = [];
        var checkRolled = rolled.concat([]);

        for(var r in rewards) {
var rewardData = rewards[r];

            if (rewardData.shapeless) {
                //Shapeless
                var rolledRight = 0;
                var neededPattern = rewardData.pattern.filter(function(item) {
                    return !!item;
                });
                var newRolled = checkRolled.concat([]);
                for(var i in checkRolled) {
var roll = checkRolled[i];
                    if (roll == null) { continue; }
                    var checkRollIndex = neededPattern.indexOf(roll);
                    if (checkRollIndex > -1) {
                        newRolled[i] = null;
                        rolledRight++;
                    }
                }
                if (rolledRight >= neededPattern.length) {
                    checkRolled = newRolled;
                    while (rolledRight >= neededPattern.length) {
                        giveRewards.push(rewardData);
                        rolledRight -= neededPattern.length;
                    }
                }
            } else {
                //Shaped
...
var roll = checkRolled[i];
                    if (roll == null) { continue; }
                    if (rewardData.pattern[i] == roll || !rewardData.pattern[i]) {
                        if (rewardData.pattern[i]) {
                            newRolled[i] = null;
                        }
                        rolledRight++;
                    }
                }
                if (rolledRight >= rollAmount) {
                    checkRolled = newRolled;
                    giveRewards.push(rewardData);
                } else if (rewardData.mirrored) { //Check mirrored pattern
                    rolledRight = 0;
                    newRolled = checkRolled.concat([]);
                    for(var i in checkRolled) {
var roll = checkRolled[i];
                        if (roll == null) { continue; }
                        var mirrorIndex = rewardData.pattern.length - (parseInt(i) + 1);
                        if (rewardData.pattern[mirrorIndex] == roll || !rewardData.pattern[mirrorIndex]) {
                            if (rewardData.pattern[mirrorIndex]) {
                                newRolled[mirrorIndex] = null;
                            }
                            rolledRight++;
                        }
                    }
        ...
        runDelay(1, function() {
            if (giveRewards.length > 0) {
                var data = e.player.world.storeddata;
                var p = new Player(e.player.getName()).init(data);
                var prizes = {};
                for(var i in giveRewards) {
var giveReward = giveRewards[i];
                    for(var type in giveReward.rewards) {
var value = giveReward.rewards[type];
                        switch (type) {
                            case 'credit':
                            case 'vmoney':
                            case 'money':
                            case 'amoney':
                                p.data[type] += value;
                                prizes[type] = (prizes[type] || 0) + value;
                                break;
                        }
                    }
                }
                ...
                for(var i in VIRTUAL_CURRENCIES) {
var currency = VIRTUAL_CURRENCIES[i];
                    if (prizes[currency.name]) {
                        tellTarget(e.player, '@a', '&a' + e.player.getName() + ' won ' + formatCurrency(prizes[currency.name], currency.name) + '&r&a for rolling &r' + subtitle);
                    }
                }
                p.save(data);

            } else {
                tellPlayer(e.player, '&cYou didn\'t win any prize. Shift + Right click to see the prizes.');
            }
        });

        tellPlayerTitle(e.player, '', 'subtitle');
        tellPlayerTitle(e.player, subtitle, 'title');
...
function getArrayRange(arr, index, rangeLeft, rangeRight) {
	if(typeof(rangeRight) == typeof(undefined) || rangeRight === null) { rangeRight = null; }
    rangeRight = (rangeRight == null ? rangeLeft : rangeRight);
    var newArr = [];
    for (var i = index - rangeLeft; i <= index + rangeRight; i++) {
        if (i < 0) {
